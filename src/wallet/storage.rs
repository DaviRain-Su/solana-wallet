...
impl WalletStorage {
    ...
    pub fn load_wallet(&self, wallet_name: &str, password: &str) -> Result<WalletData> {
        ...
        let mut key_bytes = [0u8; 32];
        if let Err(e) =
            argon2.hash_password_into(password.as_bytes(), &encrypted.salt, &mut key_bytes)
        {
            return Err(anyhow!("Failed to derive key: {:?}", e));
        }
        let key = Key::<Aes256Gcm>::from_slice(&key_bytes);
...
mod tests {
    ...
    fn test_encrypt_decrypt() {
        ...
        // Save wallet
        storage
            .save_wallet(wallet_name, &wallet_data, password)
            .unwrap();
    ...
    fn test_wrong_password() {
        ...
        storage
            .save_wallet("test", &wallet_data, "correct_password")
            .unwrap();
