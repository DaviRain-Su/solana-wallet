...
impl RpcManager {
    ...
    pub fn new(network: SolanaNetwork) -> Self {
        let client =
            RpcClient::new_with_commitment(network.rpc_url(), CommitmentConfig::confirmed());
    ...
    pub async fn request_airdrop(&self, pubkey: &Pubkey, lamports: u64) -> Result<Signature> {
        ...
        match network {
            ...
            SolanaNetwork::Mainnet => Err(anyhow!("Airdrop is not available on mainnet")),
        }
    }
}
...
mod tests {
    ...
    fn test_network_urls() {
        ...
        assert_eq!(
            SolanaNetwork::Custom("http://localhost:9999".to_string()).rpc_url(),
            "http://localhost:9999"
        );
    }
